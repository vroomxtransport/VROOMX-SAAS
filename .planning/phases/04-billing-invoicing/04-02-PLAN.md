...
<task type="auto">
  ...
  <action>
    ...
    **Step 2: Create `src/lib/pdf/invoice-template.tsx`**
    ...
    2. **Invoice Info (top-right):** "INVOICE" title, invoice number (format: `INV-{orderId}` — use the order's UUID directly, e.g., `INV-a1b2c3d4-...`), current date. Per locked decision: invoice number = `INV-{order_id}`.
...
<task type="auto">
  ...
  <action>
    **Step 1: Create `src/app/api/invoices/[orderId]/pdf/route.ts`**
    ...
    7. Returns `new Response(pdfBuffer, { headers: { 'Content-Type': 'application/pdf', 'Content-Disposition': 'inline; filename="INV-{orderId}.pdf"' } })`. Uses the order UUID as the invoice identifier per locked decision.
    ...
    **Step 2: Create `src/app/api/invoices/[orderId]/send/route.ts`**
    ...
    7. Computes invoice number: `INV-${orderId}` — use the order's UUID directly. Per locked decision: invoice number format is `INV-{order_id}`. No extraction from order_number, no regex, no fallback needed.
