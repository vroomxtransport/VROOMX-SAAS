  ...
  <action>
    ...
    **Function 1: `fetchBrokerReceivables(supabase: SupabaseClient)`**
    ...
    2. Group by broker and compute aggregations into `BrokerReceivable` objects:
       ...
       ```
    3. For `paidThisMonth`: also fetch orders with `payment_status = 'paid'` that were updated this month:
       ```typescript
       const startOfMonth = new Date()
       startOfMonth.setDate(1)
       startOfMonth.setHours(0,0,0,0)
       // Separate query or include 'paid' in the initial query with date filter
       ```
       Keep it simple: include 'paid' orders updated since start of current month in the initial query.
