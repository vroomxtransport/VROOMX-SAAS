...
must_haves:
  truths:
    - "Dashboard shows onboarding wizard when entity counts are zero AND onboarding_completed_at is null"
    - "Onboarding wizard does NOT reappear if user has dismissed it (onboarding_completed_at is set)"
    - "Onboarding wizard links to create first driver, truck, and order"
    - "Onboarding wizard has a dismiss button that sets onboarding_completed_at"
  ...
  artifacts:
    - path: "src/app/(dashboard)/dashboard/page.tsx"
      provides: "Dashboard with smart onboarding CTA gated by onboarding_completed_at AND entity counts"
      contains: "onboarding_completed_at"
    - path: "src/app/actions/onboarding.ts"
      provides: "Server Action to mark onboarding as completed (sets onboarding_completed_at)"
      exports: ["dismissOnboarding"]
  ...
  key_links:
    ...
    - from: "src/app/(dashboard)/dashboard/page.tsx"
      to: "tenants.onboarding_completed_at"
      via: "Dashboard fetches onboarding_completed_at to gate wizard display"
      pattern: "onboarding_completed_at"
    - from: "src/app/(dashboard)/dashboard/page.tsx"
      to: "src/app/actions/onboarding.ts"
      via: "Dismiss button calls dismissOnboarding Server Action"
      pattern: "dismissOnboarding"
---
...
<task type="auto">
  <name>Task 1: Dashboard onboarding wizard (gated by onboarding_completed_at), onboarding dismiss action, and layout grace period/suspension banner</name>
  <files>src/app/(dashboard)/dashboard/page.tsx, src/app/(dashboard)/layout.tsx, src/app/actions/onboarding.ts</files>
  <action>
**0. Create `src/app/actions/onboarding.ts`:**

Server Action to mark onboarding as completed by setting `onboarding_completed_at` on the tenant:
...
Replace the static Quick Start Guide card and hardcoded "0" stat cards with a smart onboarding section. The dashboard already fetches tenant data. Update the tenant select to include `onboarding_completed_at`, then add entity count queries:

Update the existing tenant select to include onboarding_completed_at:
```typescript
.select('name, plan, subscription_status, onboarding_completed_at')
```
...
Replace the Quick Start Guide card with a conditional onboarding section. Use `showOnboarding` (not `isNewUser`) to gate the wizard:
...
**Note on inline server actions in layout:** The `form action` pattern with an inline async function that imports the billing action is valid in Next.js Server Components. It avoids importing the billing action at the top level (which would always load Stripe code even when not needed).
  ...
  <verify>Run `npx tsc --noEmit`. Verify dashboard shows real entity counts. Verify onboarding wizard appears when all counts are zero AND onboarding_completed_at is null. Verify onboarding wizard does NOT appear when onboarding_completed_at is set (even if counts are zero). Verify dismiss button sets onboarding_completed_at and hides wizard. Verify grace period banner appears in layout when grace_period_ends_at is set. Verify suspension banner appears when is_suspended is true. Verify "Update Payment" buttons trigger billing portal redirect.</verify>
  <done>Dashboard shows smart onboarding CTA gated by both onboarding_completed_at (null) AND entity counts (all zero), with clickable steps linking to drivers/trucks/orders pages and a dismiss button that sets onboarding_completed_at to prevent reappearance. Stat cards show real counts. Layout shows amber grace period banner and red suspension banner with "Update Payment" buttons that redirect to Stripe Billing Portal. dismissOnboarding Server Action created in src/app/actions/onboarding.ts.</done>
</task>
...
- Dashboard shows entity counts and onboarding wizard when all counts zero AND onboarding_completed_at is null
- Dashboard does NOT show onboarding wizard when onboarding_completed_at is set (even if counts are zero)
- Dismiss button calls dismissOnboarding and wizard disappears
- Dashboard shows plan/org cards when user has existing data or has dismissed onboarding
...
- Onboarding wizard has dismiss button that permanently hides it via onboarding_completed_at
- Onboarding wizard does NOT reappear if entities are later deleted (guarded by onboarding_completed_at)
