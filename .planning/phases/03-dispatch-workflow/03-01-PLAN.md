...
must_haves:
  truths:
    ...
    - "trips table has origin_summary and destination_summary TEXT columns for route display"
...
CREATE TABLE public.trips (
  ...
  origin_summary TEXT,       -- e.g., "FL" or "FL, GA" (derived from first order's pickup_state)
  destination_summary TEXT,  -- e.g., "NY" or "NY, PA" (derived from last order's delivery_state)
  -- Metadata
  notes TEXT,
...
- Both CREATE TABLE statements present with origin_summary and destination_summary TEXT columns on trips
...
<task type="auto">
  <name>Task 2: Drizzle schema, TypeScript types, and Zod validations</name>
  <files>
    src/db/schema.ts
    src/types/index.ts
    src/types/database.ts
...
5. Add `trips` pgTable definition with all columns matching the SQL migration. Use `numeric()` with `{ precision: 12, scale: 2 }` and `.default('0')` for financial columns (string defaults for decimal precision, same as orders table). Include `originSummary: text('origin_summary')` and `destinationSummary: text('destination_summary')` columns for the denormalized route summary.
...
- Drizzle schema includes originSummary and destinationSummary columns
- Trip interface includes origin_summary and destination_summary fields
- Zod schemas export both Input and infer types
  </verify>
  <done>Drizzle schema has trips + trip_expenses tables with route summary columns. TypeScript types include TripStatus, ExpenseCategory with labels/colors/arrays. Zod schemas validate trip and expense forms. Trip interface includes origin_summary and destination_summary.</done>
</task>

<task type="auto">
  <name>Task 3: Sidebar navigation and StatusBadge updates</name>
  <files>
    src/components/layout/sidebar.tsx
    src/components/shared/status-badge.tsx
  </files>
  <action>
**A. Update `src/components/layout/sidebar.tsx`:**
...
**B. Update `src/components/shared/status-badge.tsx`:**
...
- Sidebar shows correct navigation items (Orders, Dispatch, Brokers)
- StatusBadge supports 'trip' type with correct colors and labels
  </verify>
  <done>Sidebar shows Orders, Dispatch, Brokers navigation. StatusBadge supports trip status type with correct colors and labels.</done>
</task>
...
2. SQL migration file is valid SQL with origin_summary and destination_summary columns (review manually)
3. Drizzle schema types match SQL column definitions exactly (including route summary columns)
4. Types/database.ts interfaces match Drizzle column names (snake_case), Trip has origin_summary + destination_summary
...
- trips table includes origin_summary and destination_summary TEXT columns
...
- Trip and TripExpense interfaces in types/database.ts (Trip includes origin_summary, destination_summary)
