...
- **Row columns per CONTEXT.md medium density spec:**
  ...
  5. **Route** -- Derive from first and last order pickup/delivery locations. If no orders, show "No orders". If only one order, show "pickup_state -> delivery_state". Format: "FL -> NY" (state abbreviations from first order pickup to last order delivery)
     - NOTE: Route summary requires order data. Since the trips query doesn't join orders, derive route from a simplified approach: just show start_date location info or "orders.length orders" for now. Actually, the trip doesn't store route info directly, and querying all orders for every trip row would be expensive. Instead, show `${orderCount} order(s)` and let the user click through to trip detail for route info. This keeps the dispatch board query fast.
...
- Row click navigates to /trips/[id]
  ...
  <done>Dispatch board at /dispatch shows status-grouped trip sections with filterable rows. Each row shows trip #, truck, driver, capacity, order count, status badge, and date range. Rows are clickable to navigate to trip detail.</done>
</task>
...
8. URL search params persist filter state (shareable URLs)
</verification>

<success_criteria>
- Dispatch board accessible at /dispatch with status-grouped trip sections
- Trip rows show: trip #, truck, driver, capacity, order count, status, dates
