...
must_haves:
  truths:
    ...
    - "Orders can be assigned and unassigned from trips with financial recalculation"
...
7. **`recalculateTripFinancials(tripId: string)`** (internal helper, also exported for use in trip-expenses.ts)
   ...
   - Fetch trip's orders: `orders.select('revenue, broker_fee').eq('trip_id', tripId)`
   ...
   - Update trip with denormalized values: total_revenue, total_broker_fees, driver_pay, total_expenses, net_profit, order_count
...
- Both old AND new trip recalculated on order reassignment
  ...
  <done>Trip CRUD, status workflow with order auto-sync, order assignment/unassignment with dual-trip recalculation, and expense CRUD with recalculation all implemented as Server Actions</done>
</task>
...
3. recalculateTripFinancials is called after every financial mutation
4. assignOrderToTrip recalculates BOTH old and new trips
...
- Order assignment syncs status and recalculates both trips
