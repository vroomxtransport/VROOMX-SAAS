...
```
  ...
  <verify>
    ...
    - All 3 ALTER TABLE statements present
...
Create empty placeholder directories for all subdirectories listed in the structure above (create a .gitkeep in each empty directory so Git tracks them).
  </action>
  <verify>
    Verify directory structure exists:
    - VroomXDriver/ directory at project root
    - VroomXDriverApp.swift and Config.swift exist
    - All subdirectories created (Core/, Models/, Theme/, Views/Auth/, etc.)
    - Package.swift with supabase-swift and KeychainAccess dependencies
  </verify>
  <done>Xcode project scaffold exists with directory structure, entry point, config, and dependency declarations</done>
</task>
...
- VroomXDriver/ directory structure matches research spec
...
- Xcode project scaffold provides the directory structure for all subsequent iOS plans
