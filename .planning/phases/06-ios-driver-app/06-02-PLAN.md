...
must_haves:
  truths:
    - "App data models match VroomX database schema (UUID-based IDs, snake_case column mapping)"
    - "Models match VroomX actual database schema (migrations 00002, 00003, 00006)"
    - "Theme system provides dark mode (default) and light mode with VroomX brand colors (Claude's discretion: blue #3B82F6 primary, violet #8B5CF6 accent per research recommendation)"
    - "SupabaseManager provides a singleton Supabase client"
    - "Offline status indicator available for connectivity state (NetworkMonitor tracks online/offline)"
...
**Colors.swift:**
- Extension on `Color` with VroomX brand palette (Claude's discretion per CONTEXT.md — research recommended blue/violet, NOT Horizon green):
