...
must_haves:
  truths:
    ...
    - "File uploads use InspectionUploadQueue for offline resilience and background processing"
  artifacts:
  ...
  key_links:
    ...
    - from: "VroomXDriver/Views/Orders/FileManagementGrid.swift"
      to: "VroomXDriver/Core/InspectionUploadQueue.swift"
      via: "Queues file uploads for background processing"
...
- **Upload via InspectionUploadQueue (LOCKED per CONTEXT.md offline support):** New attachments are queued via InspectionUploadQueue (or general-purpose upload queue from Plan 06-04) for background processing and offline resilience. Do NOT upload directly from the view. Queue the upload item with mediaKind='attachment', save locally first, then queue for background upload to Supabase Storage `bol-documents` bucket. On upload completion, create order_attachments record.
- Upload status indicator per file: spinner for uploading, checkmark for uploaded, exclamation for failed
...
- File grid shows existing attachments and allows adding new ones via InspectionUploadQueue (offline-resilient)
</verification>
