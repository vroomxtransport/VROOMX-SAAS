...
must_haves:
  truths:
    ...
    - "Expense creation supports receipt photo upload to Supabase Storage receipts bucket"
  artifacts:
...
- **Financial summary card** (cardBackground with border):
  ...
  - Driver Pay: trip.driver_pay (highlighted in brandPrimary — this is what the driver cares about most)
...
- **Expenses section:**
  ...
  - "Add Expense" opens a sheet with: category picker (fuel/tolls/repairs/lodging/misc), amount field (decimal keypad), notes field, date picker, **receipt photo capture button** (camera icon — opens camera or photo library), receipt thumbnail preview if photo attached, "Save" button
  - **Receipt photo upload (LOCKED per CONTEXT.md):** When a receipt photo is attached:
    1. Save photo locally to Documents directory
    2. Upload to Supabase Storage `receipts` bucket at path `{tenantId}/{tripId}/{expenseId}.jpg` (compress to 80% JPEG quality)
    3. Store the storage path in the `receipt_url` column of the trip_expenses record
    4. If offline, queue upload via PendingActionsQueue (or InspectionUploadQueue reused as general upload queue from Plan 06-04) and save receipt_url as pending
  - Expense list rows show a small receipt icon/thumbnail if receipt_url is present; tapping it opens the receipt photo in a full-screen sheet (load from Supabase Storage URL or local cache)
  - Expense creation via DataManager.createExpense (which now accepts optional receipt_url)
...
- Expense CRUD works (add via sheet, delete via swipe) with receipt photo support
- Receipt photo uploads to Supabase Storage `receipts` bucket and receipt_url stored in DB
...
- Expenses can be added (with optional receipt photo) and deleted from trip detail
- Receipt photos upload to Supabase Storage and link to expense record
