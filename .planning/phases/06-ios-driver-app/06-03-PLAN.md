...
must_haves:
  ...
  artifacts:
    ...
    - path: "VroomXDriver/Views/Auth/LoginView.swift"
      provides: "Login UI with email input, OTP verification, biometric prompt"
      contains: "LoginView"
...
<task type="auto">
  ...
  <action>
**LoginView.swift:**
- Two-phase UI: email entry -> OTP verification
...
- Set up NetworkMonitor as environment object
  ...
  <verify>
    ...
    - LoginView has email entry and OTP verification phases
  </verify>
  <done>Complete auth UI + app shell: driver can log in via email OTP, sees 5-tab navigation, can log out</done>
</task>
...
<success_criteria>
- Driver can enter email, receive OTP, verify code, and reach the 5-tab main screen
- Biometric unlock available after first successful login
