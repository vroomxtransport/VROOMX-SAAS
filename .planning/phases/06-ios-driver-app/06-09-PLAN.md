...
- **"Sign & Complete" button:** On tap:
  ...
  4. **Navigate to BOLPreviewView (Plan 10):** After successful save, set `@State var completedInspectionId: String?` which triggers a NavigationLink/fullScreenCover to `BOLPreviewView`. Pass: the completed inspection ID, order data, and inspection data needed for BOL generation. The BOLPreviewView is built in Plan 10 — for now, create a placeholder `@State var showBOLPreview = false` and `@State var completedInspectionId: String?` that Plan 10 will wire to the real BOLPreviewView.
  5. BOLPreviewView handles its own dismissal back to the order detail
